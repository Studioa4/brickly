...
export default function ModalePermessiUtente({ open, livelloCorrente, nomeUtente, onClose, onSalva }: Props) {
  const [selectedLevel, setSelectedLevel] = useState<number>(
  typeof livelloCorrente === "number" ? livelloCorrente : 1
);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        <h2 className="text-lg font-bold mb-2">
          Permessi per <span className="text-blue-600">{nomeUtente}</span>
        </h2>
